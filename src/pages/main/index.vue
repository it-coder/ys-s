<template>
  ...
  >
    <div class="container">
      <div class="tabbar-nav" v-if="platform === 'web'">
        <!-- <div class="tabbar-nav-name">{{ data.communityName }}</div> -->
      ...
      <div class="left-bg" v-if="platform === 'web'"></div>
      <div class="right-bg" v-if="platform === 'web'"></div>
      <!-- <a-modal
      ...
      >
        <div class="pop-video">
          <video-item :src="videoUrl" :index="'engine'" />
        </div>
      </a-modal> -->
...
import { queryPoints, queryVideo } from "/@/api/engine/index";
import Trending from "./trending/index.vue";
import EngineBackground from "../engine/index.vue";
...
function adjustWindowScale() {
  ...
  if (clientWidth < 483) {
    platform.value = "app";
    const scale = (clientWidth / 483).toFixed(4);
    if (unref(futureRef)) {
...
.outter[data-platform="app"] > #fixed-body {
  width: 1920px;
  height: 1080px;
  overflow: hidden;
  position: relative;
